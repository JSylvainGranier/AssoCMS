...
function fetch($url) {
    try {
        ...
        $vazStream = stream_get_meta_data($out);
        $vazPathInfo = pathinfo($url);

        return array(
            'tmp_name' => $vazStream['uri'],
            'name' => sanitize_filename($vazPathInfo['basename']),
            'type' => $type,
            'length' => $length,
...
function is_url($str) {
    ...
    $vazParu = parse_url($str, PHP_URL_SCHEME);
    return in_array($vazParu['http'], $vazParu['https'], $vazParu['ftp']);
}
