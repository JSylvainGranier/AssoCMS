...
class Transfer {
    ...
    public function getFiles($mutator = null) {
        ...
        $files = array_merge(isset($this->file) ? array($this->file) : array(), $this->variants);
        return $mutator === null ? $files : call_user_func($mutator, $files, $this->metadata);
    }
    
    public function restore($file, $variants = array(), $chunks = array(), $metadata = array()) {
        $this->file = $file;
    ...
    public function populate($entry) {
        ...
        $metadata = isset($_POST[$entry]) ? to_array($_POST[$entry]) : array();
